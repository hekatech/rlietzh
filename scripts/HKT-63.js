...
$('.article-date').each(function(){
 var dateText = $(this).html().replace(/[\n\s]/g, '').split(',');
  var dateMonth = Number(dateText[0]),
  deteDay = dateText[1],
  deteYear = dateText[2];
  const monthNames = ["Jan.", "Feb.", "Mar.", "Apr.", "May", "June", "July", "Aug.", "Sept.", "Oct.", "Nov.", "Dec."];
  var str = monthNames[dateMonth - 1] + " " + deteDay + " \/ " + deteYear;
  $(this).html(str);
});

// more

$(".article-entry #more").html('. ʚ + <span>✦</span> + ɞ .');
...
if ($('#category-list').length > 0) {
  $(document).ready(function() {
          $("#category-list").prependTo("#cat-box");
      })
  };